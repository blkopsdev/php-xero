...
class ExampleClass
{
	...
	public function getAccount($xero)
	{
		...
		//[/Accounts:Read]
		$str = $str . "Get all Accounts Total: " . count($accounts) . "<br>";

		// GET ACCOUNT with WHERE CLAUSE
		$accountWhere = $xero->load('Accounting\\Account')->where('
		...
		if (count($accountWhere)) {
			$str = $str . "Get an Account where Type is BANK: " . $accountWhere[0]["Name"] . "<br>";
		} else {
			$str = $str ."No Account of Type BANK found";					
		}

		return $str;
	}

	public function createAccount($xero,$returnObj=false)
	{
		$str = '';
	...
	public function updateAccount($xero)
	{
		$str = '';
		...
		$str = $str . "Update Account: " . $new["Name"] . " -- " . $new["Description"] . "<br>" ;

		return $str;
	}

	public function archiveAccount($xero)
	{
		$str = '';
		...
		$str = $str . "Archive Account: " . $account["Name"] . "<br>" ;

		return $str;
	}

	public function deleteAccount($xero)
	{
		$str = '';
	...
	{	
		...
		$where = $xero->load('Accounting\\BankTransaction')->where('
	...
	public function createBankTransaction($xero,$returnObj=false)
	{
		$str = '';

		$lineitem = $this->getLineItem($xero);
		...
		if (count($account)) {
			...
			$banktransaction = new \XeroPHP\Models\Accounting\BankTransaction($xero);
			$banktransaction->setReference('Ref-' . $this->getRandNum())
				->setDate(new DateTime('2017-01-02'))
				->setType(\XeroPHP\Models\Accounting\BankTransaction::TYPE_RECEIVE)
	...
	public function updateBankTransaction($xero)
	{
		$str = '';

		$new = $this->createBankTransaction($xero,true);
		$guid = $new['BankTransactionID'];

		//[BankTransactions:Update]
		$banktransaction = $xero->loadByGUID('Accounting\\BankTransaction', $guid);
		$banktransaction->setReference('Ref-' . $this->getRandNum())
			->setSubTotal(NULL)
			->setTotal(NULL);
		$banktransaction->save();
		...
		$str = $str . "Updated Bank Transaction: " . $banktransaction["Reference"] . " -- Total: " . $banktransaction["Total"] . "<br>" ;

		return $str;
	}

	public function deleteBankTransaction($xero)
	{
		$account = $this->getBankAccount($xero);
		...
		if (count($account)) {
			...
			$banktransaction = $xero->loadByGUID('Accounting\\BankTransaction', $guid);
			$banktransaction->setStatus(\XeroPHP\Models\Accounting\BankTransaction::BANK_TRANSACTION_STATUS_DELETED)
				->setSubTotal(NULL)
				->setTotal(NULL);
			$banktransaction->save();
			//[/BankTransactions:Delete]
	...
	public function getBankTransfer($xero)
	{
		$str = '';

		//[BankTransfers:Read]
		$banktransfers = $xero->load('Accounting\\BankTransfer')->execute();
		//[/BankTransfers:Read]

		$str = $str . "Get BankTransfers: " . count($banktransfers) . "<br>";
